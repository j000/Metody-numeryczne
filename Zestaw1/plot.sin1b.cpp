...
const MyFloat czebyszew
	= PI / 4 * (1. + std::cos((2. * I + 1.) / (2. * N + 2.) * PI));

static inline const Point data[]{{0., 0.},
								 {czebyszew<2, 2>, std::sin(czebyszew<2, 2>)},
								 {czebyszew<1, 2>, std::sin(czebyszew<1, 2>)},
								 {czebyszew<0, 2>, std::sin(czebyszew<0, 2>)},
								 {PI / 2, 1.}};
...
MyFloat sin1(const MyFloat x)
{
	...
	if (x <= PI)
		return sin1(PI - x, data, data_size);
	if (x <= 2 * PI)
		return -sin1(x - PI);
